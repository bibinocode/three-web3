...
export default class Physics {
  constructor(planeGroup, camera, scene) {
    ...
    const geometry = new THREE.CircleGeometry(0.35)
    const matetrial = new THREE.MeshBasicMaterial({
      color: 0xfff000,
      side: THREE.DoubleSide
    })
    // this.capsuleBodyControl = new THREE.Mesh(geometry, matetrial)
  ...
  playerCollisions () {
    ...
    this.playerOnFloor = false;
    if (result) {
      // 这里不知道因为什原因，原本是大于0，可能是模型的问题，所以我改成了-0.35。要不然会一直抖动
      this.playerOnFloor = result.normal.y > -0.35;
